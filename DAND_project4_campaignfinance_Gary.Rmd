...
oh$entity = ifelse(grepl("PARTNERS|CONSULTING|LLP|LLC|GROUP", oh$contbr_nm), 
                   "company", "individual") 
...
oh = merge(oh, ohio_zip[, c("contbr_zip" ,"latitude", "longitude")], 
           all.x=TRUE)
...
ggplot(data = dat2, aes(x = cand_last, y = money_received))+
  ...
  coord_flip()+
  scale_y_continuous(limits = c(0, max(dat2$money_received)+250000), 
                     label = comma)+
  scale_fill_manual(values=party_colors)+
  geom_text(aes(label = format(round(money_received, 0), big.mark = ",")), 
            ...
            size = 3,
            hjust = -0.2)+
  ggtitle("Total Donations Recevied by Candidates")
```
...
```{r, echo= F, warning= F}
top_runners = c("Clinton", "Sanders", "Kasich", 
                "Rubio", "Carson", "Bush", "Cruz")
...
dat4$cand_last = factor(dat4$cand_last, 
                        levels = dat4$cand_last[order(dat4$avg_donation_amt, 
                                                      decreasing =T)])
...
```{r, echo = F}
ggplot(data = oh, aes(x = contb_receipt_dt, 
                      y = contb_receipt_amt, 
                      label = contbr_nm, 
                      color = party))+
  geom_point(alpha = 1/2, 
              aes(size = oh$contb_receipt_amt,
                  shape = factor(oh$party)))+
    ...
    geom_text(
      aes(
        label=ifelse(oh$contb_receipt_amt >=5500,
                     paste0(oh$contbr_nm," $",
                            oh$contb_receipt_amt, " to ",
                            oh$cand_last),''),angle = 10),
      hjust=1.08,
      size = 2,
      color = "black")+
  labs(shape="Party", color="Party", size = "Donation Amount")+
  geom_hline(aes(yintercept = 5000), color = "purple", linetype="dashed")+
  annotate("text", label = "Donation $5000", 
           x = as.Date("2014-07-31"), y = 5500, size = 4, colour = "grey20")+
...
```{r, echo = F}
p5 = ggplot(
  data = subset(oh, contb_receipt_amt<= quantile(contb_receipt_amt, .75)), 
...
ggplot(data = don_gen_ideo,
       aes( x = contbr_gender, y = party, fill = avg_donations)) +
  geom_tile()+
  scale_fill_continuous("Avg. Donation ($)", high = "steelblue", low = "orangered")+
  labs(title = "Donations to Liberal/Conservative by Gender", 
       x = "Donors Gender",
       y = 'Ideology')+
  geom_text(aes(label= paste0("$", don_gen_ideo$avg_donations)),
            size=5, 
            color = "grey25") 
...
```{r, echo = F}
oh$cand_gender = ifelse(oh$cand_last %in% c("Clinton", "Fiorina"), 
                        "female_candidate", 
                        "male_candidate" )
...
ggplot(data = don_gen_gender,
  ...
  scale_fill_continuous("Avg. Donation ($)", 
                        high = "steelblue", 
                        low = "orangered")+
  labs(title = "Donations to Male/Female Candidates by Gender", 
       x = "Donors Gender",
       y = 'Candidates Gender')+
  geom_text(aes(label= paste0("$", don_gen_gender$avg_donations)),
            size=5,
            color = "grey25") 
```
...
ggplot() + 
  geom_polygon(data=ohio, aes(x=long, y=lat, group = group),
  ...
  labs(title = "Donors Geographical Distribution") +
  scale_alpha_continuous(name = element_blank(), labels = "donors")
```
...
#Ohio census data
pop = read.csv("/Users/garymu/Dropbox/Udacity/DAND/project4/ohio_census.csv", 
               stringsAsFactors = F)
...
#add in geo info 
don_by_region = merge(don_by_region, 
                      ohio, 
                      by.x = 'county', 
                      by.y = 'subregion', 
                      all.x = TRUE)
...
key_counties = data.frame(
  county = c("Hamilton", "Franklin", "Cuyahoga", 'Lucas', 
             "Adams", "Monroe", "Belmont"),
...
p8 = ggplot(data=ohio, aes(x=long, y=lat, group = group),
       colour="black") + 
  geom_polygon(data = don_by_region, 
               aes(fill= cut_number(don_by_region$donation_per_cap,5)))+
  scale_fill_brewer("Donations Per Capita by County",
                    palette  = 'RdYlGn', 
                    label = comma)+
  theme_bw()+
...
```{r echo=FALSE, fig.width = 8, fig.height= 6.5,Plot_Three}
don_by_region2 = oh2 %>%
  group_by(county) %>%
  summarise(
...
p9 = ggplot(data=ohio, aes(x=long, y=lat, group = group),
       colour="black") + 
  geom_polygon(data = don_by_region, 
               aes(fill= cut_number(don_by_region$donation_per_cap,5)))+
  scale_fill_brewer("Donations Per Capita by County",
  ...
  geom_text(data = key_counties , 
            ...
            size = 4, 
            color = "black") +
  geom_point(data = don_by_region2, 
             color = "grey3",
             alpha = 1/2,
             aes(x = longitude, 
                 y = latitude,
                 group = NULL,
                 size = cut_number(don_by_region2$total_donation,5)))

p9 +scale_size_discrete(name = "Total Donation Amt")
```
